...
function Seatlayout() {
  ...
  const handleSeatClick=(seatId)=>{
    ...
    if(!selectedSeats.includes(seatId)&&selectedSeats.legth >4){
    ...
    try {
      if(!user)return toast.error('Please login to proceed')
        if(!selectedTime||!selectedSeats.length)return toast.error('please select a time and seats')
          const {data}=await axios.get('/api/booking/create',{showId:selectedTime.showId
            ,selectedSeats
        ...
        {Array.from({length:count},(_,i)=>{
          ...
          return(
            <button key={seatId} onClick={()=>handleSeatClick(seatId)} className={`h-8 w-8 rounded
              border border-primary/60 cursor-pointer ${selectedSeats.includes(seatId)&& "bg-primary text-white"
  ...
  return show ? (
    <div className='flex flex-col md:flex-row px-6 md:px-16 lg:px-40 py-30 md:pt-50'>
      ...
      max-md:mt-16'>
       ...
       <button onClick={bookTicket} className='flex items-center gap-1 mt-20 px-10 py-3 text-sm 
